...
export default function CourseCard({ course }: Props) {
  ...
  return (
    <Link
      ...
      className="group block cursor-pointer body-text overflow-hidden rounded-[32px] border border-black/5 bg-white shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl"
    >
      {/* Thumbnail */}
      <div className="relative h-60  overflow-hidden">
        ...
        {/* Badge */}
        <div className="absolute bottom-5 body-text left-5 rounded-full bg-white/20 px-4 py-2 text-xs font-medium text-white backdrop-blur-md">
          Premium Course
        </div>
      </div>
      ...
      <div className="p-6">
        ...
        <div className="mb-4 flex items-center gap-3">
          ...
          <div>
            <p className="text-sm body-text text-gray-500">Instructor</p>

            <h3 className="font-semibold body-text">{course.teacher?.name}</h3>
          </div>
        </div>

        {/* Title */}
        <h2 className="line-clamp-2 body-text font-bold text-2xl font-bold leading-tight">
          {course.title}
        </h2>

        {/* Description */}
        <p className="mt-4 line-clamp-3 body-text text-sm leading-relaxed text-gray-500">
          {course.description}
        </p>
